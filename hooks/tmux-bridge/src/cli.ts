...
function ensureProvider(provider: string, session?: string): void {
  ...
  if (entry) {
    if (paneExists(entry.paneId)) return;
...
async function main(): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'ask': {
      ...
      const purpose = (askArgs['purpose'] ?? 'general') as import('./types.js').Purpose;
      const meta: Record<string, string> = {};
      if (askArgs['base']) meta['base'] = askArgs['base'];
      ...
      if (askArgs['wait']) {
        ...
        if (result.status !== 'done') process.exit(1);
      } else {
    ...
    case 'status': {
      ...
      if (args['provider']) {
        const found = Object.values(all).some(
          (entry: any) => entry.provider === args['provider']
        );
        if (!found) process.exit(1);
        const entries = Object.fromEntries(
          Object.entries(all).filter(([, e]: [string, any]) => e.provider === args['provider'])
        );
        console.log(JSON.stringify(entries, null, 2));
      } else {
        console.log(JSON.stringify(all, null, 2));
        if (Object.keys(all).length === 0) process.exit(1);
      }
      break;
    }
