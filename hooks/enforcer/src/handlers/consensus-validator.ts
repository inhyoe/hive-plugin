...
export function extractConsensusInputFromStdin(stdin: string): ConsensusInput | null {
  try {
...
/**
 * Track round numbers per team to detect gaps and enforce max 5 rounds.
 * Uses .hive-state/consensus-rounds.json as persistent storage.
 */
interface RoundTracker {
  [teamId: string]: number[];
}

function loadRoundTracker(stateDir: string): RoundTracker {
  const filePath = join(stateDir, 'consensus-rounds.json');
  if (!existsSync(filePath)) return {};
  try {
    return JSON.parse(readFileSync(filePath, 'utf-8')) as RoundTracker;
  } catch {
    return {};
  }
}

function saveRoundTracker(stateDir: string, tracker: RoundTracker): void {
  if (!existsSync(stateDir)) mkdirSync(stateDir, { recursive: true });
  writeFileSync(
    join(stateDir, 'consensus-rounds.json'),
    JSON.stringify(tracker, null, 2),
    'utf-8',
  );
}

export function handleConsensusValidator(
