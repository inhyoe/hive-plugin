 ...
 */
export function handleReadGatePre(stateDir: string): HandlerResult {
 ...
 */
export function handleReadGatePost(
  stdin: string,
  stateDir: string,
  repoRoot: string,
): HandlerResult {
...
function extractReadPath(stdin: string): string | null {
  try {
...
function isReadError(stdin: string): boolean {
  try {
    const parsed = JSON.parse(stdin);
    ...
    if (typeof result.stdout === 'string' && result.stdout.includes('Error reading file')) return true;
    return false;
  } catch {
    return false;
  }
