...
1. Wave별 실행 (순서 중요 — tmux-bridge async guardrail 준수):
   ...
   Step B: tmux-bridge 호출 — tmux-ask.sh codex (파일 내용 + 구체적 수정 지시 + round_id)
           -> HIVE_ASYNC_SUBMITTED 시 턴 종료
   Step C: 다음 턴에서 pend 수집 후, tmux-ask.sh gemini (테스트/문서 작업)
   필수: 대규모(6+) Codex 최소 2개, 중소(3-5) 최소 1개 모듈 직접 구현

2. 결과 수집:
   - Claude: SendMessage 수신
   - tmux-bridge: pend 수집 (HIVE_DONE marker 확인)
