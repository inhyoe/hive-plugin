...
<hard_gate rule="PHASE5_ENTRY">
1. 진입 검증: Bash("bash $HIVE_PLUGIN_DIR/scripts/validate-phase5-entry.sh") → FAIL 시 진입 금지
2. G4-G7 TDD 게이트 강제 (참조: hive-tdd-pipeline §2-5)
</hard_gate>
### 5-1. 실행 순서 (Wave 기반)
...
**실행 순서**: Claude Agent tool 호출을 먼저 실행 (병렬 스폰), 이후 tmux-bridge /ask 호출.
